...
| Market data | yfinance | 15-min delayed, free, no rate limits in practice. Also powers **`get_option_chain`** (077) — read-only options chain (strikes/IV/OI, ~15-min delayed, **no Greeks**), rendered as a markdown table (no widget). `USE_MOCK_OPTIONS` mock-first. |
