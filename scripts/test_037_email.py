"""Offline test for 037 — email briefing channel (SendGrid).

Fully offline: never sends, never hits the DB/network. The real SendGrid path is
...
monkeypatched to async stubs.
    ...
    backend/.venv/bin/python scripts/test_037_email.py
"""
...
# 037 modules; the repo backend (has ibkr_flex.py) is the fallback for the rest.
...
async def test_scheduler_email_leg() -> None:
    ...
    _set("SENDGRID_API_KEY", "SG.realish")
